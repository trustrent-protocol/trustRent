...
const create = (data) => pool.query(
  ...
  [
    data.lease_id, data.amount, data.asset || 'USDC', data.memo || null,
    data.tx_hash || null, data.ledger || null, data.status || 'pending',
    data.splits ? JSON.stringify(data.splits) : null,
    data.settled_at || null,
  ]
);

const findByLease = (lease_id) => pool.query(
  'SELECT * FROM payments WHERE lease_id = $1 ORDER BY created_at DESC', [lease_id]
);

const findByTxHash = (tx_hash) => pool.query(
  'SELECT * FROM payments WHERE tx_hash = $1', [tx_hash]
);

const confirm = (id, tx_hash, ledger, splits, settled_at) => pool.query(
