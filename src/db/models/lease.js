...
const create = (data) => pool.query(
  ...
  [
    data.landlord_id, data.tenant_id, data.agent_id || null,
    data.property_address, data.rent_amount, data.deposit_amount,
    data.asset || 'USDC', data.agent_fee_pct || 0, data.lease_hash || null,
    data.starts_at, data.ends_at, data.duration_months,
  ]
);

const findById = (id) => pool.query('SELECT * FROM leases WHERE id = $1', [id]);

const updateStatus = (id, status) => pool.query(
  `UPDATE leases SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *`,
  [status, id]
);

const setEscrowAccount = (id, escrow_account_pk) => pool.query(
